    ...
    void ShowBoat() {
		...
		if (Physics.Raycast(input_ray, out hit)) {                      //mouse -> tile
            ...
            //for(int i = 0; i < colliders.Length; i++){
                ...
                Ray testing_ray = new Ray(boat.transform.position + Vector3.Scale(item.center, boat.transform.localScale), Vector3.down*100);
                RaycastHit testing_hit;
                //Debug.DrawRay(boat.transform.position + Vector3.Scale(colliders[i].center, boat.transform.localScale), Vector3.down*100, Color.green, 1000f, false);
